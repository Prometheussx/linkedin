...
if st.button("Profil Tarama ve Görsel Analizi Yap"):
    if search_query and page_nos:
        ...
        for index, row in df_results.iterrows():
            cols = st.columns([1, 5, 2])  # Kolon genişlikleri ayarlandı
            with cols[0]:
                st.write(row['Index'])
            with cols[1]:
                st.write(row['Name'])
...
Estetik International Kliniği
                """
                # Metni textarea içinde göster
                st.text_area("Kopyalanabilir Metin:", value=message, height=300)

               
                    

        # Yorumları gösterme
        st.write("Saç Sorunu Analizi Yorumları:")
        st.write(yorum)
